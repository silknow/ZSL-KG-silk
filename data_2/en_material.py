...
df = pd.read_csv('en_material.csv', low_memory=False)
...
df = df.groupby(['obj'], as_index=False)['museum','text','material_group'].agg(lambda x: list([i for i in x if pd.notna(i)]))
...
df.to_csv('en_material_post2.csv', index=False)
